...
export const playWinSound = (winnings: number, cost: number): void => {
  ...
  try {
    ...
    if (!AudioContextClass) {
      console.warn("Web Audio API is not supported in this browser. Cannot play sound.");
      return;
    }
    ...
    const closeDelayMs = (scheduledEndTime - audioContext.currentTime + 0.2) * 1000; // Delay in ms + 200ms buffer
    if (closeDelayMs > 0) {
        setTimeout(() => {
    ...
    console.error("Could not initialize or play win sound:", e instanceof Error ? e.message : String(e));
    // Attempt to clean up context if creation failed mid-way (optional, error handling specific)
    if (audioContext && audioContext.state !== 'closed') {
      audioContext.close().catch(err => console.error("Error closing audio context after failure:", err));
    }
  }
};