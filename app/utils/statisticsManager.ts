...
export async function fetchStatistics(): Promise<StatisticsData | null> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch statistics: HTTP status ${response.status}`);
    }
    ...
    if (!isValidStatisticsData(statsData)) {
      ...
      console.error('Fetched statistics data has invalid structure:', JSON.stringify(statsData));
    ...
    console.error('Error fetching or processing statistics:', error instanceof Error ? error.message : String(error));
