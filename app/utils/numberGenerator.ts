...
import { MAIN_NUMBER_MAX, EURO_NUMBER_MAX, MAIN_NUMBER_MIN, EURO_NUMBER_MIN } from './constants';
...
export function generateNumbers(
    ...
    if (stats && stats.length > 0) {
        try {
            ...
            console.warn(`Weighted generation failed: ${error instanceof Error ? error.message : String(error)}. Falling back to random generation.`);
...
function generateNumbersWithStatsInternal(
    ...
    if (!stats || stats.length === 0) {
        throw new Error('No valid statistics data provided for weighted generation.');
    }
    ...
    if (cumulativeDistribution.length === 0 || !isFinite(totalAdjustedValue) || totalAdjustedValue <= 0) {
    ...
    if (selectedNumbers.size < count) {
        console.warn(`Could only select ${selectedNumbers.size}/${count} unique numbers using weighted stats after ${count * 10} attempts. Filling the remainder randomly.`);
        ...
        if (selectedNumbers.size < count) {
             throw new Error(`Failed to fill remaining ${remainingCount} numbers randomly.`);
        }
    }
...
export function generateRandomNumbers(count: number, min: number, max: number): number[] {
    ...
    if (rangeSize < count) {
        throw new Error(`Cannot generate ${count} unique numbers from a range of size ${rangeSize} (${min}-${max})`);
    }
    if (count <= 0) {
        return [];
    }
