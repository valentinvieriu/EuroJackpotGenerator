...
import { H3Event, H3Error, createError, readBody, defineEventHandler } from 'h3';
...
export default defineEventHandler(async (event: H3Event): Promise<Ticket[]> => {
  try {
    ...
    const ticketCount = parseInt(String(ticketCountInput), 10);
    const mainCount = parseInt(String(mainCountInput), 10);
    const euroCount = parseInt(String(euroCountInput), 10);

    // Validate ticketCount
    if (isNaN(ticketCount) || ticketCount < MIN_TICKETS || ticketCount > MAX_TICKETS) {
      throw createError({
        statusCode: 400,
    ...
    // Validate mainCount
    if (isNaN(mainCount) || mainCount < MIN_MAIN_COUNT || mainCount > MAX_MAIN_COUNT) {
       throw createError({
        statusCode: 400,
     ...
     // Validate euroCount
    if (isNaN(euroCount) || euroCount < MIN_EURO_COUNT || euroCount > MAX_EURO_COUNT) {
       throw createError({
        statusCode: 400,
    ...
    console.log(`Generating ${ticketCount} tickets with system ${mainCount}/${euroCount}...`);
    ...
    throw createError({
      statusCode: 500,
      statusMessage: 'An internal server error occurred while generating tickets.',
