...
function toAbsoluteDocsUrl(href) {
  try {
    ...
    if (href.startsWith('/en/docs/claude-code/')) return `https://docs.anthropic.com${href}`
...
function htmlToMarkdownLite(html) {
  ...
  // headings
  html = html.replace(/<h1[^>]*>([\s\S]*?)<\/h1>/gi, (_, t) => `# ${t.replace(/<[^>]+>/g, '').trim()}\n\n`)
  html = html.replace(/<h2[^>]*>([\s\S]*?)<\/h2>/gi, (_, t) => `## ${t.replace(/<[^>]+>/g, '').trim()}\n\n`)
  html = html.replace(/<h3[^>]*>([\s\S]*?)<\/h3>/gi, (_, t) => `### ${t.replace(/<[^>]+>/g, '').trim()}\n\n`)
  html = html.replace(/<h4[^>]*>([\s\S]*?)<\/h4>/gi, (_, t) => `#### ${t.replace(/<[^>]+>/g, '').trim()}\n\n`)
  ...
  // inline code
  html = html.replace(/<code[^>]*>([\s\S]*?)<\/code>/gi, (_, code) => '`' + code.replace(/<[^>]+>/g, '').trim() + '`')

  // lists
  html = html.replace(/<li[^>]*>([\s\S]*?)<\/li>/gi, (_, t) => `- ${t.replace(/<[^>]+>/g, '').trim()}\n`)
  html = html.replace(/<\/ul>|<\/ol>/gi, '\n')

  // paragraphs/line breaks
  html = html.replace(/<br\s*\/?>/gi, '\n')
  html = html.replace(/<p[^>]*>([\s\S]*?)<\/p>/gi, (_, t) => `${t.replace(/<[^>]+>/g, '').trim()}\n\n`)
...
async function ensureOutputDir(filePath) {
  const dir = path.dirname(path.isAbsolute(filePath) ? filePath : path.join(__dirname, filePath))
...
function extractSidebarLinks(htmlContent) {
  ...
  let groupMatch = htmlContent.match(/<ul[^>]*id=["']sidebar-group["'][^>]*>([\s\S]*?)<\/ul>/i)
  ...
  if (!groupMatch) {
    ...
    groupMatch = ulMatches.find(m => /href="[^"]*\/en\/docs\/claude-code\//i.test(m[1]))
  }

  if (!groupMatch) {
    log.warning('Could not find a clear "first sidebar group"; falling back to scanning entire page.')
    // final fallback: scan whole page
    const allLinks = [...htmlContent.matchAll(/href="([^"]*\/en\/docs\/claude-code\/[^"#?]+)[^"]*"/gi)].map(m => m[1])
    const abs = uniquePreserveOrder(allLinks.map(toAbsoluteDocsUrl).map(stripAnchorAndTrailingSlash))
    log.info(`Found ${abs.length} candidate links via fallback scan`)
    return abs
  }

  const firstGroupHtml = groupMatch[1]
  const links = [...firstGroupHtml.matchAll(/href="([^"]*claude-code[^"]*)"/gi)].map(m => m[1])
  const abs = uniquePreserveOrder(links.map(toAbsoluteDocsUrl).map(stripAnchorAndTrailingSlash))
...
async function fetchMarkdownFor(url) {
  ...
  try {
    ...
    try {
      ...
      const filename = path.basename(url)
      log.success(`Converted HTML -> MD-lite (${mdLite.length} bytes) from ${filename}`)
      return {
        filename,
        content:
`> **Note:** This section was converted from HTML because \`.md\` was not available for this page.\n\n${mdLite}`,
      }
    } catch (err) {
