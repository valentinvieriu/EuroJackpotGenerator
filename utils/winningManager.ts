...
export function calculateTotalWinnings(
  ...
  if (!winningData?.eurojackpotOdds || winningData.eurojackpotOdds.length === 0) {
    console.warn("Cannot calculate winnings: Winning data or eurojackpotOdds array is missing or empty.");
  ...
  winningData.eurojackpotOdds.forEach(odd => {
    ...
    if (Number.isInteger(odd.winningClass) && odd.winningClass >= 1 && odd.winningClass <= 12 && typeof odd.amount === 'number' && odd.amount >= 0) {
       oddsMap.set(odd.winningClass, odd.amount);
    } else {
        console.warn(`Invalid or missing data for winning class entry: ${JSON.stringify(odd)}. Skipping.`);
    }
  });
  ...
  tickets.forEach(ticket => {
    ...
    if (ticket.winClass && Number.isInteger(ticket.winClass) && ticket.winClass >= 1 && ticket.winClass <= 12) {
      ...
      if (oddsMap.has(ticket.winClass)) {
        ...
        totalWinnings += oddsMap.get(ticket.winClass)!;
          ...
          console.warn(`Ticket ${ticket.id} has winClass ${ticket.winClass}, but no corresponding amount found in odds data.`);
      }
    }
